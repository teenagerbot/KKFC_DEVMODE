    ...
    output.on('close', function () {
        ...
        if (!file_system.existsSync("resources/export")) {
            file_system.mkdir("resources/export", err => {
                ...
                file_system.rename("resources/DB.zip", "resources/export/DB.zip", err => {
                    if (err) {
...
const creator = (server) => {
    remote.dialog.showMessageBox(windows, {
        "type": "question",
        'title': 'Оберіть дію',
        'message': "База була успішно експортована, вона знаходиться за наступним шляхом: 'resources/export/DB.zip', що ви хочете далі зробити?",
        'buttons': [
        ...
        if (result.response === 0) {
            ...
            shell_manager.showItemInFolder(remote.app.getAppPath().toString().replace("\\app.asar", "\\export\\DB.zip"));
        }
    });
}
