...
function Load() {
   ...
   */
  document.querySelector('input:first-child').value = object[0]['Назва деталі'];
  document.querySelector('td:nth-child(2) input').value = object[0]['Матеріал'];
  document.querySelector('td:nth-child(3) input').value =
    object[0]['Маса деталі, кг'];
  document.querySelector('td:nth-child(4) input').value =
    object[0]['Тип заготівки'];
  document.querySelector('td:nth-child(5) input').value =
    object[0]['Маса заготівки'];
  document.querySelector('td:nth-child(6) input').value =
    object[0]['Розмір заготівки B, мм'];
  document.querySelector('td:nth-child(7) input').value =
    object[0]['Розмір заготівки H, мм'];
  document.querySelector('td:nth-child(8) input').value =
    object[0]['Кількість'];
}
Load();
/**
 //FIXME - Додати на всі сторінки свої системні кнопки
 //FIXME - Додати ефект завантаження(свг)
 */
clear('__data_FORM2__');

//FIXME - Створити елементи для змінювання та зберігання
